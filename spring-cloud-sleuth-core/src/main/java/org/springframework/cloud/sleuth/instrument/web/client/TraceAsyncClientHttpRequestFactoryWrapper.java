 ...
 * {@link AsyncClientHttpRequest} and to the {@link ClientHttpRequest}
 *
...
public class TraceAsyncClientHttpRequestFactoryWrapper extends AbstractTraceHttpRequestInterceptor
	...
	private static final XTraceLogger xtrace = XTrace.getLogger(TraceAsyncClientHttpRequestFactoryWrapper.class);
	 ...
	 * {@link AsyncClientHttpRequestFactory} implement the
	 * {@link ClientHttpRequestFactory} interface.
	 *
	 * In case that it's not true we're setting the
	 * {@link SimpleClientHttpRequestFactory} as a default for sync request
	 * processing.
	 *
	...
	public TraceAsyncClientHttpRequestFactoryWrapper(Tracer tracer, SpanInjector<HttpRequest> spanInjector,
			AsyncClientHttpRequestFactory asyncDelegate, HttpTraceKeysInjector httpTraceKeysInjector) {
		super(tracer, spanInjector, httpTraceKeysInjector);
		this.asyncDelegate = asyncDelegate;
		this.syncDelegate = asyncDelegate instanceof ClientHttpRequestFactory ? (ClientHttpRequestFactory) asyncDelegate
				: defaultClientHttpRequestFactory();
	}

	/**
	 * Default implementation that creates a
	 * {@link SimpleClientHttpRequestFactory} that has a wrapped task executor
	 * via the {@link TraceAsyncListenableTaskExecutor}
	 */
	public TraceAsyncClientHttpRequestFactoryWrapper(Tracer tracer, SpanInjector<HttpRequest> spanInjector,
	...
	public AsyncClientHttpRequest createAsyncRequest(URI uri, HttpMethod httpMethod) throws IOException {
		...
		xtrace.log("createAsyncRequest (before)", request);
		publishStartEvent(request);
		xtrace.log("createAsyncRequest (with baggage)", request);
		return request;
	}
	...
	public ClientHttpRequest createRequest(URI uri, HttpMethod httpMethod) throws IOException {
		...
		addRequestTags(request);
		xtrace.log("createRequest (before)", request);
		publishStartEvent(request);
		xtrace.log("createRequest (with baggage)", request);
		return request;
	}
}
