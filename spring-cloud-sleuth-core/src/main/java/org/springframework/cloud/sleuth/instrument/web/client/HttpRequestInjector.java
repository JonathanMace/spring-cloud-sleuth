...
class HttpRequestInjector implements SpanInjector<HttpRequest> {
	...
	public void inject(Span span, HttpRequest carrier) {
		...
		if (zb == null) {
			zb = new ZipkinBaggage();
		}
		zb.traceID = span.getTraceId();
		zb.spanID = span.getSpanId();
		zb.sampled = span.isExportable() ? true : false;
		if (zb.sampled) {
			zb.parentSpanID = getParentId(span);
		}
		// TODO: process ID and span name name here if we choose to add them.
		
		ZipkinBaggage.set(zb);
